        ...
        if (isOpen) {
            ...
            if (!current.voidElement && nextChar && nextChar !== '<') {
                current.children.push({
                    type: 'text',
                    content: unescapeString(html.slice(start, html.indexOf('<', start)))
                });
            }
    ...
    tag.replace(attrRE, function (match) {
        if (i % 2) {
            ...
            if (i === 0) {
                ...
                res.attrs[key] = unescapeString(match.replace(/['"]/g, ''));
            }
        }
        i++;
    });
...
function unescapeChar(chr) {
  return unescapes[chr];
}

// unescaped special chars in string (handlerbar escaped chars)
function unescapeString(str) {
    if (!unescapeRE.test(str)) {
        return str;
    } else {
        return str.replace(unescapeRE, unescapeChar);
    }
}
